...
@main
struct YolculukApp: App { var body: some Scene { WindowGroup { WebContainer().ignoresSafeArea(.container, edges: .bottom) } } }
...
struct WebContainer: UIViewRepresentable {
    ...
    final class Coordinator:NSObject,WKNavigationDelegate,WKUIDelegate,WKDownloadDelegate {
        ...
        func download(_ download:WKDownload,decideDestinationUsing response:URLResponse,suggestedFilename:String,completionHandler:@escaping(URL?)->Void){let u=FileManager.default.temporaryDirectory.appendingPathComponent(suggestedFilename.isEmpty ? "Datei":suggestedFilename);try? FileManager.default.removeItem(at:u);downloadURL=u;completionHandler(u)}
